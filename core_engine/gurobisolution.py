...
def load_dataset(filepath):
    """Solomon TXT dosyasından verileri çeker (Kapasite satırını zekice atlar)."""
    customers = []
    weights = []
    data_lines = []
...
# ==========================================
# 2. GUROBI İLE ZİYARET SIRASI (PHASE 1)
# ==========================================

def solve_tsp_gurobi(customer_coords, depot_coord, drone_speed):
    print("Phase 1: Gurobi TSP Çözücüsü Başlatılıyor...")
    all_nodes = [depot_coord] + customer_coords
    num_nodes = len(all_nodes)
    
    model = gp.Model("Drone_TSP")
    model.Params.OutputFlag = 0 
    
    x = model.addVars(num_nodes, num_nodes, vtype=GRB.BINARY, name="x")
    u = model.addVars(num_nodes, vtype=GRB.CONTINUOUS, name="u")
    
    obj = gp.quicksum((math.dist(all_nodes[i], all_nodes[j]) / drone_speed) * x[i, j]
                      for i in range(num_nodes) for j in range(num_nodes) if i != j)
    model.setObjective(obj, GRB.MINIMIZE)
    
    model.addConstrs((x.sum(i, '*') == 1 for i in range(num_nodes)), "Cikis")
    model.addConstrs((x.sum('*', j) == 1 for j in range(num_nodes)), "Giris")
    
    # KANKA İŞTE GUROBİ'NİN HİLE YAPMASINI ENGELLEYEN YENİ KİLİT KOD BURASI:
    model.addConstrs((x[i, i] == 0 for i in range(num_nodes)), "KendineGitme")
    
    for i in range(1, num_nodes):
        for j in range(1, num_nodes):
            if i != j:
                model.addConstr(u[i] - u[j] + num_nodes * x[i, j] <= num_nodes - 1)
    ...
    if model.status == GRB.OPTIMAL:
        ...
        while True:
            ...
            for j in range(num_nodes):
                if current_node != j and x[current_node, j].X > 0.5:
                    next_node = j
                    break
        ...
        print(f"Gurobi Başarılı! Ziyaret Sırası: {visit_order + [0]}")
        return visit_order
    else:
        raise Exception("Gurobi çözüm bulamadı!")

# ==========================================
# 3. BLOK ATAMA VE SHORTEST PATH (PHASE 2 & 3)
# ==========================================

def run_rts_algorithm(visit_order, V_locations, customer_coords, customer_weights, k, EMAX, drone_speed, truck_speed):
    print("Phase 2: Block Partitioning ve Enerji Hesabı Yapılıyor (Dinamik Drone Seçimi Aktif)...")
    G = nx.DiGraph()
        ...
        for i2 in V_locations.keys():
            ...
            for j1 in range(total_customers + 1):
                for j2 in range(j1, total_customers + 1):
                    ...
                    if j1 == j2:
                        G.add_edge((i1, j1), (i2, j2), weight=truck_time, details="Sadece Kamyon Hareket Etti")
                        continue
                    ...
                    # Görevleri biriktireceğimiz liste (EKSİK OLAN BUYDU)
                    drone_tasks = [] 
                    ...
                    for d in range(k):
                        ...
                        # DİNAMİK DRONE SEÇİMİ: Toplam ağırlığı bul ve uygun modeli seç
                        ...
                        # Yapılan işi listeye ekle
                        drone_tasks.append(f"Drone {d+1} ({drone_name} - {total_weight:.2f}kg) -> Paketler: {drone_packages}")
                    ...
                    if is_feasible:
                        ...
                        # RAPORU OLUŞTUR VE DETAY (details) OLARAK GRAFİĞE KAYDET
...
def plot_routes(depot_coord, customer_coords, visit_order, best_path, V_locs):
    ...
    # 3. Drone Paket Dağıtım Sırasını Çiz (Gurobi'nin Çıkardığı TSP Rotası - Kesik Gri Çizgi)
    all_nodes = [depot_coord] + customer_coords
    tsp_x = [all_nodes[i][0] for i in visit_order]
    tsp_y = [all_nodes[i][1] for i in visit_order]
    plt.plot(tsp_x, tsp_y, linestyle=':', color='gray', alpha=0.7, label='Drone Dağıtım Sırası (TSP)')

    # 4. Kamyon Rotasını Çiz (Faz 3'te Bulunan Kamyonun Fiziksel Durakları - Kalın Siyah Çizgi)
    truck_x = []
    truck_y = []
    for node in best_path:
        physical_node = node[0] # node formatı: (fiziksel_konum, teslim_edilen_paket_indexi)
        truck_x.append(V_locs[physical_node][0])
        truck_y.append(V_locs[physical_node][1])

    plt.plot(truck_x, truck_y, linestyle='-', linewidth=2.5, color='black', label='Kamyon Rotası')

    # Harita Ayarları
    plt.title('Multi-Visit Drone Routing Problem - Optimizasyon Haritası', fontsize=14, weight='bold')
